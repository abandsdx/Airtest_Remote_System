...
app.get('/api/recordings', authRequired, (req, res) => {
  res.json({ recordings: store.data.recordings });
});

app.get('/api/recordings/:id/stream', authRequired, (req, res) => {
  const record = store.data.recordings.find((item) => item.id === req.params.id);
  if (!record) {
    return res.status(404).json({ error: 'not_found' });
  }
...
setInterval(() => {
  ...
  operators.forEach((ws) => {
    if (ws.readyState !== ws.OPEN) {
      operators.delete(ws);
      devices.forEach((entry) => entry.watchers.delete(ws));
      return;
    }
...
function updateDeviceState(deviceId, online, info) {
  ...
  const patch = { online };
  if (typeof info?.streaming === 'boolean') {
    patch.streaming = info.streaming;
  }
  if (typeof info?.camera === 'boolean') {
    patch.camera = info.camera;
  }
  if (typeof info?.mic === 'boolean') {
    patch.mic = info.mic;
  }
  store.updateDeviceStatus(deviceId, patch);
...
function startRecording(deviceId, actor) {
  const entry = devices.get(deviceId);
  ...
  const fileName = `${deviceId}-${Date.now()}.rdr`;
  ...
  const record = {
    ...
    deviceId,
    path: filePath,
    startedAt: entry.recording.startedAt,
    createdAt: Date.now(),
    updatedAt: Date.now(),
    createdBy: actor,
    size: 0,
  ...
  store.addRecording(record);
  broadcastToOperators({ type: 'recording-started', recording: record });
  return record;
}

function stopRecording(deviceId, actor) {
  const entry = devices.get(deviceId);
  if (!entry || !entry.recording) {
    return null;
  }
  ...
  const record = store.updateRecording(recording.id, {
    endedAt: Date.now(),
  ...
  if (record) {
    broadcastToOperators({ type: 'recording-stopped', recording: record });
  }
  return record;
}

function recordPacket(deviceId, data) {
  const entry = devices.get(deviceId);
  if (!entry || !entry.recording) {
    return;
  }
  ...
  entry.recording.stream.write(lengthBuffer);
  entry.recording.stream.write(data);
  entry.recording.bytes += 4 + data.length;
}
...
wssDevice.on('connection', (ws, req) => {
  ...
  ws.on('message', (data, isBinary) => {
    ...
    recordPacket(deviceId, buffer);

    const entry = devices.get(deviceId);
    if (!entry) return;

    if (wsFrameDebug && ws._debugRecvCount < 5 && buffer.length > 0) {
      console.log(`WS device recv packet: type=${buffer[0]} bytes=${buffer.length}`);
    }

    if (buffer.length > 0) {
      const packetType = buffer[0];
      if (packetType === 1) {
        entry.streamConfigs.screen = buffer;
      } else if (packetType === 3) {
        entry.streamConfigs.camera = buffer;
      } else if (packetType === 5) {
        entry.streamConfigs.audio = buffer;
      }
    }

    entry.watchers.forEach((client) => {
...
wssOperator.on('connection', (ws, req) => {
  ...
  ws.on('message', (data, isBinary) => {
    ...
    if (['control', 'service', 'run_task'].includes(msg.type)) {
